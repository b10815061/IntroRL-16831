...
class RL_Trainer(object):
    ...
    def run_training_loop(self, n_iter, collect_policy, eval_policy,
        ...
        ## store expert for displaying expert movement
        ...
        for itr in range(n_iter):
            ...
            print('******* log video: ' , self.log_video, '*******')
            ...
            b4 = np.sum([path['action'].sum() for path in paths])
            if relabel_with_expert and itr>=start_relabel_with_expert:
                paths = self.do_relabel_with_expert(expert_policy, paths)  # HW1: implement this function below
            after = np.sum([path['action'].sum() for path in paths])
            print('b4 vs after:', b4, after, np.allclose(b4,after))
    ...
    def train_agent(self):
        ...
        for train_step in range(self.params['num_agent_train_steps_per_iter']):
            ...
            # print('observation max length is: ' , len(self.agent.replay_buffer.obs))
    ...
    def perform_logging(self, itr, paths, eval_policy, train_video_paths, training_logs):
        print('******* This is a discrete work?' , self.params['agent_params']['discrete'], '*******')
        ...
        # self.logger.log_paths_as_videos(eval_video_paths, itr, fps=self.fps,max_videos_to_save=MAX_NVIDEO,
        #                                      video_title='eval_rollouts')
        # expert_video_paths = utils.sample_n_trajectories(self.env,self.expert_policy, MAX_NVIDEO, self.MAX_VIDEO_LEN, True)
        # self.logger.log_paths_as_videos(expert_video_paths, itr, fps=self.fps,max_videos_to_save=MAX_NVIDEO,
        #                                      video_title='expert_rollouts')
        # print(len(eval_video_paths))  
        # print(len(expert_video_paths))                                            
        # print('video saved!')
        ...
        if self.log_metrics:
            ...
            logs['n_iter'] = self.params['n_iter']
            logs['eval_batch_size'] = self.params['eval_batch_size']
            logs['batch_size'] = self.params['batch_size']
            logs['train_batch_size'] = self.params['train_batch_size']
            logs['n_layers'] = self.params['n_layers']
            logs['size'] = self.params['size']
            logs['learning_rate'] = self.params['learning_rate']
