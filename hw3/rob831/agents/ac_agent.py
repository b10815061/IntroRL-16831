...
class ACAgent(BaseAgent):
    ...
    def train(self, ob_no, ac_na, re_n, next_ob_no, terminal_n):
        ...
        loss = OrderedDict()
        ## optimize critic network by
        ## 1. update targets with current value function
        ## 2. try to fit on target with some grad steps

        for _ in range(self.agent_params['num_critic_updates_per_agent_update']):
            ## should not use += here ... because we want to record how well the best estimator do
            ## using += accounts for some past estimator loss too
            loss['Loss_Critic'] = self.critic.update(ob_no,ac_na,next_ob_no,re_n,terminal_n)
        ## calculate advantage by the trained critic network
    ...
    def estimate_advantage(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        Vsp = self.critic(next_ob_no)
        Q = re_n + self.gamma*Vsp*(not terminal_n)
        adv_n = Q - Vs

        if self.standardize_advantages:
            adv_n = normalize(adv_n,adv_n.mean(1),adv_n.std(1))
        return adv_n
