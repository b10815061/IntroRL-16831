...
class FFModel(nn.Module, BaseModel):
    ...
    def forward(
        ...
        delta_pred_normalized = self.delta_network(concatenated_input) ## norm for stable training

        ## need unnormed here to map back to real world
        delta_pred_unnormed = unnormalize(delta_pred_normalized, delta_mean, delta_std)
        next_obs_pred = obs_unnormalized + delta_pred_unnormed
    ...
    def get_prediction(self, obs, acs, data_statistics):
        ...
        # outputs.
        ## should be next_obs_pred here as we need to predict the next ob. Regardless of delta.
        return prediction
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        # Hint: `self(...)` returns a tuple, but you only need to use one of the
        # outputs.
        loss = self.loss(target,normed_delta)
